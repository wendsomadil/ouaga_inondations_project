...
points = [
    ...
    {"lat":12.350765, "lon":-1.587388, "name":"Zone E : ST DOMINIQUE","contact":"M. Dao",           "comment":"Caniveaux débordés menaçant rez‑de‑chaussée.",       "images":["images/stdcdom1.jpg","images/stdcdom2.jpg"]},
...
def risk_map():
    ...
    if 'classe' in grid.columns:
        folium.Choropleth(geo_data=grid, data=grid,
                          ...
                          fill_color='YlOrRd', legend_name='Niveau de risque (1–5)').add_to(m)
    return m
...
def zonage_map():
    ...
    if 'classe' in grid.columns:
        ...
        for _,r in grid.iterrows():
            col=colors.get(r['classe'],'gray')
            folium.GeoJson(r.geometry, style_function=lambda f,c=col:{'fillColor':c,'fillOpacity':0.4,'color':'none'}).add_to(m)
    return m
